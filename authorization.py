  ...
  def __init__(self, secrets_file=CLIENT_SECRETS):
    if os.path.isfile(secrets_file):
      ...
      if CLIENT_ID not in input_json:
        raise ValueError(f"Input JSON must have a \"{CLIENT_ID}\" field")
      if CLIENT_SECRET not in input_json:
        raise ValueError(f"Input JSON must have a \"{CLIENT_SECRET}\" field")
...
class ApiAccess():
  ...
  def __init__(self, client_data=ClientData(), secrets_file=EPHEMERAL_SECRETS):
    ...
    if os.path.isfile(secrets_file):
      ...
      if REFRESH_TOKEN not in input_json:
        raise ValueError(f"Input JSON must have a \"{REFRESH_TOKEN}\" field")
      self.refresh_token = input_json[REFRESH_TOKEN]
      self.expiration_time = int(input_json[EXPIRATION_TIME]) if EXPIRATION_TIME in input_json else 0
      self.access_token = input_json[ACCESS_TOKEN] if ACCESS_TOKEN in input_json else None
    else:
      self.refresh_token = input("Please enter your current Refresh Token: ")
  ...
  def __refresh_credentials(self):
    ...
    if expiration_time < current_time:
      if self.expiration_time == 0:
        print("No access token expiration time available; attempting to retrieve access token.")
      else:
        print(f"Access token expired at {expiration_time}. Current time is {current_time}")
      ...
      self.__handle_token_response(requests.post(OAUTH_TOKEN_URL, json=refresh_request, timeout=5))
  ...
  def __handle_token_response(self, response):
    ...
    print(f"Successfully retrieved new access token which expires at {datetime.fromtimestamp(self.expiration_time)}.")
    ...
    with open(EPHEMERAL_SECRETS, 'w') as f:
      json.dump({REFRESH_TOKEN: self.refresh_token, ACCESS_TOKEN: self.access_token, EXPIRATION_TIME: self.expiration_time}, f, indent=2)

  def make_request(self, url: str, method=requests.get, attempt_auth=True, url_prefix=API_URL, **kwargs):
    ...
    **kwargs: Passed to method().
    ...
    self.__refresh_credentials()
    response = method(url_prefix + url, headers={"Authorization" : f"Bearer {self.access_token}"}, **kwargs)
    if response.status_code == 401 and attempt_auth:
      ...
      return self.make_request(url, method=method, attempt_auth=False, **kwargs)
    else:
      response.raise_for_status()
    return response
  ...
  def __attempt_oauth(self):
    ...
    print("Attempting to expand scope of authorization by opening a browser window. Select whichever scopes you consider appropriate then return to this application. To avoid accidents, this will not attempt to request write access.")
    ...
    class HttpRequestHandler(http.server.BaseHTTPRequestHandler):
      def do_GET(self):
        self.send_headers()
        self.wfile.write("Success!\nYou may close this tab and return to the command-line.".encode('utf-8'))
        # server.shutdown() cannot be called from the same thread as the
        # server itself without causing a deadlock.
        shutdown_thread = Thread(target=lambda server: server.shutdown(), args=(local_server,))
        shutdown_thread.start()
        # Should really not have this be nonlocal, but this whole server is a mess.
        nonlocal oauth_result
        oauth_result = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
      def do_HEAD(self):
        self.send_headers()
    ...
    LOCAL_PORT = 8001
    local_server = http.server.HTTPServer(('', LOCAL_PORT), HttpRequestHandler)
    ...
    webbrowser.open("http://www.strava.com/oauth/authorize?%s" % urllib.parse.urlencode({"client_id": self.client_data.client_id, "redirect_uri": f"http://localhost:{LOCAL_PORT}", "response_type": "code", "approval_prompt": "force", "scope": ",".join(scopes)}))
    ...
    self.__handle_token_response(requests.post(OAUTH_TOKEN_URL, json=token_exchange, timeout=5))
    print(f"Successfully got authorization for scopes: {oauth_result['scope']}")
